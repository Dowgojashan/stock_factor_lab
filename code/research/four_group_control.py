...
def _portfolio_series(wide: pd.DataFrame, members: list[str]) -> pd.Series:
    ...
    🔴 **這行取橫斷面平均，隱含「每個月把權重拉回等權」**（不是買進後放著）。
    本專案全部組合績效都走這條路徑（H-12／H-26/27 矩陣／M-03 對照皆是），
    所以它是一個**貫穿全部結果的隱含假設**，論文必須揭露。

    **實測影響**（M-08，2026-09-05，OOS 窗 2019-01~2025-12、B_all 全宇宙）：

        每月再平衡 vs buy-and-hold 的 CAGR
          TW  17.5924% vs 17.7707%   → 再平衡**低** 0.178 pp
          US  25.3124% vs 27.0030%   → 再平衡**低** 1.691 pp
          XM  22.5239% vs 23.4054%   → 再平衡**低** 0.882 pp
        MDD 幾乎相同（差 < 0.04 pp）

    ⚠️ **方向是扣分不是加分**——本資料的贏家會繼續贏，每月再平衡等於一直賣掉贏家。
    （開發待辦追蹤曾寫成「既是報酬來源也是隱含成本」，實測後確認在本資料上**只有成本**。）

    ⚠️ 因為四組對照與所有矩陣格子都採同一慣例，**組間的相對比較不受影響**；
    受影響的是報給老師的**絕對報酬數字**。
    """
