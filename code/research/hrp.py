...
# ============================================================================
# 有效獨立賭注數（H-09：回答「N個策略實際上等於幾個獨立賭注」）
# ============================================================================

def effective_number_of_bets(corr: np.ndarray) -> float:
    """Meucci (2009) / López de Prado (2016) 的PCA熵版「有效獨立賭注數」（ENB）。

    對相關矩陣做特徵分解，把特徵值正規化成機率分布 p_i = λ_i / Σλ_i
    （相關矩陣 trace = N，λ全部相等時 p_i = 1/N），取該分布的**熵的指數**
    當「有效維度」：
        全部完全獨立（corr=I）      → 熵=ln(N) → ENB=N（上限，N個真獨立賭注）
        全部完美相關（corr全為1）   → 只有1個非零特徵值吃下全部trace，熵=0 → ENB=1（下限）
    介於兩者之間，數字越接近N代表這批策略彼此獨立性越高、實際承擔的風險
    來源越多元；越接近1代表看起來是N個策略，實際上只是同一個賭注的N個複製品。

    這是本專案「HRP到底幫你少了多少東西」的量化答案，也是「免費午餐」大小的
    理論上限——不管怎麼挑選子集，能拿到的分散效果都不可能超過整個宇宙的ENB。
    """
    eigvals = np.linalg.eigvalsh(corr)
    eigvals = np.clip(eigvals, 0.0, None)   # PSD矩陣理論上非負，浮點誤差可能給極小負值
    total = eigvals.sum()
    if total <= 0:
        return 0.0
    p = eigvals / total
    p = p[p > 0]                            # entropy的 0*ln(0) 定義為0，直接濾掉避免log(0)
