# -*- coding: utf-8 -*-
"""idea①：HRP挑代表時改用CAGR排序（不除以MDD），取代現行的Calmar排序，測試
是不是真的能移除§3.6/3.7查出的「窄持股→MDD壓縮→Calmar虛高」這個偏誤管道。

直接對應老師原話（H-10 docstring）：「這每一群裡面我到底要用績效最好的咧？
還是績效去除以MDD咧？」——Calmar是後者，這裡補測前者。

範圍限定（跟排除V1當初一樣，先只測一個維度）：只換quality metric
（calmar vs cagr_only），**不合併排除V1**，baseline（保留V1）跟兩邊都用
silhouette_is現場選k，anchored window4／rolling window6各測一次。

輸出：候選池組成（估值佔比/V1比例/平均持股/市值前10%佔比）＋OOS績效
（CAGR/MDD/Sharpe），比照§3.8/3.9的呈現方式。

用法（cwd 必須是 code/；PYTHONIOENCODING=utf-8）：
    PYTHONIOENCODING=utf-8 python -m app._design_test_cagr_vs_calmar_ranking
"""
...
ROLLING_WINDOW_NO = 6
QUALITY_METRICS = ["calmar", "cagr_only"]
...
def run_one(label: str, is_start: str, is_end: str, oos_start: str, oos_end: str,
           quality_metric: str, months_long, meta_pool, f_combo_map, idx: pd.DataFrame,
           mktcap: pd.DataFrame, md: MarketData):
    print(f"\n{'='*60}\n{label}／{quality_metric}：IS {is_start}~{is_end}")
    ...
    if quality_metric == "calmar":
        quality_is = cagr_is / mdd_is.abs().replace(0, np.nan)
    elif quality_metric == "cagr_only":
        quality_is = cagr_is
    else:
        raise ValueError(quality_metric)
    ...
    print(f"k={k}｜目標{tot}檔｜實選{len(a_members)}檔（backfill={n_bf}）")
    print(f"估值佔比={val_frac:.1%}｜V1比例={v1_frac:.1%}｜平均持股={avg_hold:.1f}檔｜市值前10%佔比={frac_top10:.1%}")
    print(f"IS CAGR={perf['is_cagr']:+.2%} MDD={perf['is_mdd']:.2%}｜"
         f"OOS CAGR={perf['oos_cagr']:+.2%} MDD={perf['oos_mdd']:.2%} Sharpe={perf['oos_sharpe']:.2f}")

    return {"label": label, "quality_metric": quality_metric, "k": k, "target_total": tot,
           "n_members": len(a_members), "val_frac": val_frac, "v1_frac": v1_frac,
           "avg_holdings": avg_hold, "frac_top10pct_mktcap": frac_top10,
           "is_cagr": perf["is_cagr"], "is_mdd": perf["is_mdd"],
...
def main():
    ...
    for metric in QUALITY_METRICS:
        rows.append(run_one("anchored_w4", ANCHORED_IS_START, ANCHORED_IS_END,
                            ANCHORED_OOS_START, ANCHORED_OOS_END, metric,
                            months_long, meta_pool, f_combo_map, idx, mktcap, md))
    for metric in QUALITY_METRICS:
        rows.append(run_one("rolling_w6", r_is_start, r_is_end, r_oos_start, r_oos_end, metric,
                            months_long, meta_pool, f_combo_map, idx, mktcap, md))

    out = pd.DataFrame(rows)
    out_path = Path(__file__).resolve().parent.parent.parent / "_analysis_outputs_applayer" / "cagr_vs_calmar_ranking.csv"
    out.to_csv(out_path, index=False, encoding="utf-8-sig")
    print(f"\n寫入 {out_path}")

    print("\n=== 總覽：calmar vs cagr_only ===")
    print(out[["label", "quality_metric", "n_members", "val_frac", "v1_frac", "avg_holdings",
