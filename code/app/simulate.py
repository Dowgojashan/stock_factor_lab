...
from app import actions, diagnose, facts_lean, monitor, triggers
...
def run_quarter(inputs: QuarterInputs, *, arm: str, as_of: str, end: str,
                ...
                memory: dict, excess_history: list[float], dry_run: bool = True) -> dict:
    ...
    這段期間表現如何）。呼叫端不可混用。
    """
    weights_as_of = resolve_weights(inputs.md, inputs.idx, inputs.uids, as_of)
    weights_end = resolve_weights(inputs.md, inputs.idx, inputs.uids, end)
    ...
    proc = monitor.process_layer(weights_end, mcap_row_end, prev_weights=prev_weights,
                                 member_uids=inputs.uids, candidate_idx=inputs.idx)
...
def run_simulation(run_id: str, *, model: str, api_key: str, dry_run: bool = True,
    ...
    inputs = QuarterInputs(md=md, md_map=md_map, idx=idx, mcap_wide=mcap_wide,
                           uids=uids, m1d_cond=cond, model=model, api_key=api_key,
    ...
    for arm in ARMS:
        ...
        for i, end in enumerate(quarters):
            ...
            result = run_quarter(inputs, arm=arm, as_of=as_of, end=end,
                                 ...
                                 memory=agent_a_memory, excess_history=excess_history,
                                 dry_run=dry_run)
