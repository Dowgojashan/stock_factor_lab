...
def main():
    ...
    market = args.market
    label = f"{market}_L4_M" if args.variant == "strict" else f"{market}_L4_{args.variant}_M"

    allowed, _ = load_allowed_pairs(market, args.variant)
    ...
    log(f"=== Phase 4 加入 V｜市場={market}｜in-sample 至 {IN_SAMPLE_END} ===")
