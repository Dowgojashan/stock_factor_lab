...
>    - 🔴 **v1.2 方向釐清**：**淘汰條件是「低 EffN ＋ 高 `rotation_score`」**（高分＝前三大貢獻股年年同一批＝不輪動＝靠運氣）。研究部 v9 的尾端硬篩表原本寫成「低＋低」，方向相反、已更正。
>    - ⚠️ **v1.2 實測**：62.4% 的策略此值恰為 0（P95=0.017／P99=0.025／max=0.072），**分位切點必須用 P95 以上**，否則會退化成 0。本欄位是 §五-3「一律 groupby(market) 切三段」的例外。
...
| ~~`coverage_ratio`~~ | — | 🔴 **v1.2 停用**：實測算不出此語意，且為市場常數（台 0.622–0.629／美 0.710–0.714）| — |
| **`empty_ratio`** | 0–1 | **`company_count == 0` 的比例**（明確空手、無歧義）| 快篩 |
| `eval_month_ratio` | 0–1 | 有部位紀錄的月份 ÷ 總月份跨度（＝換股頻率，**非**空手率）| 情報 |
...
**剩下唯一會真正砍到東西的是 Gate A 的「靠單一飆股」（低 EffN ＋ **高** `rotation_score`）與 C-4 的 `empty_ratio`**，而這兩個是資料品質防線，不是減量手段。
