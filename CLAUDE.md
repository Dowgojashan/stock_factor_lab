...
**應用層（`code/app/`，2026-09-08~09 新增）**：Phase A~D 完成，`replay` 模式可跑（CLI + Streamlit UI），
`live` 模式暫緩。詳見 `文件/應用層開發追蹤.md`。
...
### 🔴 2026-09-09 決定：換機器時順便脫離 XAMPP，改用獨立安裝的 MariaDB Server
...
### 2026-08-20 發生過的意外：MySQL 壞過、已修好（沿用 MariaDB 的話這段知識仍適用）
...
⚠️ **以下是舊機器的路徑，換機器後會失效，屆時請跟使用者確認新位置**：
...
讀 PDF 用 `fitz`（PyMuPDF，已在 `requirements_clean.txt` 裡），`pdftoppm` 沒裝所以 `Read` 工具的
PDF 頁面渲染會失敗，要用 `python -c "import fitz; ..."` 直接抓文字。
